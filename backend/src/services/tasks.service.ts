...
export async function createCompanyTask(
  ...
  if (assignedToId && assignedToId !== actor.actorId) {
    await createUserTargetedAlerts({
      ...
      code: "TASK_ASSIGNED",
      message: `Une nouvelle tâche ${profile.label} vous a été assignée : ${created.title}`,
      severity: "INFO",
      entityType: "TASK",
      entityId: created.id,
      metadata: {
  ...
  if (actor.role === "EMPLOYEE") {
    await createRoleTargetedAlerts({
      ...
      message: `Une nouvelle tâche a été créée par ${created.createdByFullName} : ${created.title}`,
      severity: "INFO",
      entityType: "TASK",
      entityId: created.id,
      metadata: {
...
export async function assignCompanyTask(
  ...
  if (nextAssignedToId && nextAssignedToId !== actor.actorId) {
    ...
    await createUserTargetedAlerts({
      ...
      code: "TASK_ASSIGNED",
      message: note
        ? `Une tâche vous a été assignée : ${updated.title}. Note : ${note}`
        : `Une tâche vous a été assignée : ${updated.title}`,
      severity: "INFO",
      entityType: "TASK",
      entityId: updated.id,
      metadata: {
...
export async function updateCompanyTaskStatus(
  ...
  if (input.status === "BLOCKED") {
    await createRoleTargetedAlerts({
      ...
      excludeUserIds: [actor.actorId],
      code: "TASK_BLOCKED",
      message: `Une tâche ${profile ? profile.label : "opérationnelle"} est bloquée et requiert une attention management : ${updated.title}`,
  ...
  if (input.status === "DONE" && updated.createdById !== actor.actorId) {
    await createUserTargetedAlerts({
      ...
      code: "TASK_DONE",
      message: `Une tâche que vous avez créée a été terminée : ${updated.title}`,
      severity: "INFO",
      entityType: "TASK",
      entityId: updated.id,
      metadata: {
